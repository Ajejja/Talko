import User from "../models/user.model.js";
import Message from "../models/message.model.js";
import cloudinary from "../lib/cloudinary.js";
import { getReceiverSocketId, io } from "../lib/socket.js";
import ChannelMessage from "../models/channelMessage.model.js";

export const getUsersForSidebar = async (req, res) => {
  try {
    const loggedInUserId = req.user._id;
    const filteredUsers = await User.find({ _id: { $ne: loggedInUserId } }).select("-password");

    res.status(200).json(filteredUsers);
  } catch (error) {
    console.error("Error in getUsersForSidebar: ", error.message);
    res.status(500).json({ error: "Internal server error" });
  }
};

export const getMessages = async (req, res) => {
  try {
    const { id: userToChatId } = req.params;
    const myId = req.user._id;

    const messages = await Message.find({
      $or: [
        { senderId: myId, receiverId: userToChatId },
        { senderId: userToChatId, receiverId: myId },
      ],
    });

    res.status(200).json(messages);
  } catch (error) {
    console.log("Error in getMessages controller: ", error.message);
    res.status(500).json({ error: "Internal server error" });
  }
};

export const sendMessage = async (req, res) => {
  try {
    const { text } = req.body;
    const { id: receiverId } = req.params;
    const senderId = req.user._id;

    let imageUrl = null;
    let fileUrl = null;

    if (req.files?.image) {
      const imageUpload = await cloudinary.uploader.upload(req.files.image.tempFilePath);
      imageUrl = imageUpload.secure_url;
    }

    if (req.files?.file) {
      const fileUpload = await cloudinary.uploader.upload(req.files.file.tempFilePath, {
        resource_type: "raw",
      });
      fileUrl = fileUpload.secure_url;
    }

    const newMessage = new Message({
      senderId,
      receiverId,
      text,
      image: imageUrl,
      file: fileUrl,
    });

    await newMessage.save();

    const receiverSocketId = getReceiverSocketId(receiverId);
    if (receiverSocketId) {
      io.to(receiverSocketId).emit("newMessage", newMessage);
    }

    res.status(201).json(newMessage);
  } catch (error) {
    console.log("Error in sendMessage controller: ", error.message);
    res.status(500).json({ error: "Internal server error" });
  }
};
// POST a channel message
export const sendChannelMessage = async (req, res) => {
  try {
    const { text } = req.body;
    const { channelId } = req.params;

    const newMessage = new ChannelMessage({
      text,
      channelId,
      senderId: req.user.id,
      image: req.file && req.file.mimetype.startsWith("image")
        ? `/uploads/${req.file.filename}`
        : undefined,
      file: req.file && !req.file.mimetype.startsWith("image")
        ? `/uploads/${req.file.filename}`
        : undefined,
    });

    await newMessage.save();
    const populatedMessage = await newMessage.populate("senderId", "-password");
    res.status(201).json(populatedMessage);
  } catch (err) {
    console.error("Error saving channel message:", err);
    res.status(500).json({ error: "Could not save channel message" });
  }
};

// GET all messages in a channel
export const getChannelMessages = async (req, res) => {
  try {
    const { channelId } = req.params;
    const messages = await ChannelMessage.find({ channelId }).populate("senderId", "-password");
    res.status(200).json(messages);
  } catch (err) {
    console.error("Error getting channel messages:", err);
    res.status(500).json({ error: "Could not get messages" });
  }
};